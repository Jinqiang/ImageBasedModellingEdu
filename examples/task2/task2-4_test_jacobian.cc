              ...
              double* point_x_ptr, double* point_y_ptr)
{
    ...
    // 相机径向畸变的偏导数
    double du_ddistort = f * x;
    double dv_ddistort = f * y;
    
    double ddistort_dk0 = r2;
    double ddistort_dk1 = r2*r2;
    ...
    // 相机平移向量的偏导数
    double du_dxc, du_dyc, du_dzc;
    double dv_dxc, dv_dyc, dv_dzc;

    double ddistort_dxc = (k0 + 2*k1*r2)*2*x/zc;
    double ddistort_dyc = (k0 + 2*k1*r2)*2*y/zc;
    double ddistort_dzc = -(k0 + 2*k1*r2)*2*r2/zc;
    
    double du_dx  = f * distort;
    double dv_dy  = f * distort;

    double dx_dxc = 1/zc;  double dx_dyc = 0;       double dx_dzc = -x/zc;
    double dy_dxc = 0;     double dy_dyc = 1/zc;    double dy_dzc = -y/zc;
      
    du_dxc = du_ddistort * ddistort_dxc + du_dx * dx_dxc;
    du_dyc = du_ddistort * ddistort_dyc + du_dx * dx_dyc;
    du_dzc = du_ddistort * ddistort_dzc + du_dx * dx_dzc;

    dv_dxc = dv_ddistort * ddistort_dxc + dv_dy * dy_dxc;
    dv_dyc = dv_ddistort * ddistort_dyc + dv_dy * dy_dyc;
    dv_dzc = dv_ddistort * ddistort_dzc + dv_dy * dy_dzc;
    ...
    // 相机旋转矩阵的偏导数
    double R0X = R[0]*X[0] + R[1]*X[1] + R[2]*X[2]; //R[0,:]*X
    double R1X = R[3]*X[0] + R[4]*X[1] + R[5]*X[2]; //R[1,:]*X
    double R2X = R[6]*X[0] + R[7]*X[1] + R[8]*X[2]; //R[2,:]*X

    double dxc_dw0 = 0;      double dxc_dw1 = R2X;    double dxc_dw2 = -R1X;
    double dyc_dw0 = -R2X;   double dyc_dw1 = 0;      double dyc_dw2 = R0X;
    double dzc_dw0 = R1X;    double dzc_dw1 = -R0X;   double dzc_dw2 = 0;
    ...
    cam_y_ptr[6] = dv_dyc * dyc_dw0 + dv_dzc * dzc_dw0;//0.0;
    cam_y_ptr[7] = dv_dxc * dxc_dw1 + dv_dzc * dzc_dw1;//0.0;
    cam_y_ptr[8] = dv_dxc * dxc_dw2 + dv_dyc * dyc_dw2;//0.0;

    // 三维点的偏导数

    double dxc_dX = R[0];    double dxc_dY = R[1];    double dxc_dZ = R[2];
    double dyc_dX = R[3];    double dyc_dY = R[4];    double dyc_dZ = R[5];
    double dzc_dX = R[6];    double dzc_dY = R[7];    double dzc_dZ = R[8];
